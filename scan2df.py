...
class scan2df:
    ...
    def __init__(self, image_path):
        ...
        self.thresh = cv2.threshold(self.gray, 170, 255, cv2.THRESH_BINARY)[1]
...
for f in [f for f in os.listdir() if f.endswith('.jpg')]:
    self = scan2df(f)
    dfs.append(self.ocr())
    
df = pd.concat([dfs[3], dfs[0], dfs[1], dfs[2]])


for x in [70, 90, 110, 130, 150, 170, 190]:
    self.thresh = cv2.threshold(self.gray, x, 255, cv2.THRESH_BINARY)[1]
    cv2.imwrite(f'thresh{x}.png', self.thresh)
'''